...
def train_(train_set,test_set,layer_name, q, mask_bit_position = [1]*16, epochs = 10):
    ...
    optimizer = optim.SGD(network.parameters(), lr=0.1, momentum= 0.9, weight_decay=0.0005)
            ...
            for n, m in network.named_modules():
                if isinstance(m,PrunedConv):
                    if layer_num in layer_name:
                        t_prune,t_pre,t_convert,t_mul = m.prune_by_percentage(mask_bit_position=mask_bit_position, q = q)
                        list_time += np.array([t_prune,t_pre,t_convert,t_mul])
                    layer_num +=1
...
#         print("time for masking:" + str(t_mask))
#         print("time for train:" + str(t_train))
#         print("time for test:" + str(t_test))
#         print(list_time)
        scheduler.step()
        
    return acc

# acc = train_(train_set,test_set,layer_name=[layer_name],mask_bit_position = mask_bit_position, 
#                          q=q,epochs = 100)
list_q = [1]
list_mask = [1]
list_acc = []
