...
list_mask = [1]
list_acc = []
for batch_size_train in [1280*2,1280*3]:
    for layer_name in list_layers:
        for q in list_q:
            for j in list_mask:
                mask_bit_position = [0]*16
                mask_bit_position[j] = 1 
                acc = train_(train_set,test_set,layer_name=layer_name,batch_size_train=batch_size_train, mask_bit_position = mask_bit_position, q=q,epochs = 10)
                list_acc += [acc]
        print(list_acc)
...
print(df)
## save to xlsx file
filepath = './results/lent_weights_bit1_batch_size.csv'
