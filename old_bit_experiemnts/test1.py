...
def train_(train_set,test_set,layer_name, q, mask_bit_position = [1]*16, epochs = 10):
    ...
    optimizer = optim.SGD(network.parameters(), momentum = 0.9, lr=0.1, weight_decay=0.0005)
...
for layer_name in list_layers:
    for q in list_q:
        for j in list_mask:
            ...
            acc = train_(train_set,test_set,layer_name=[layer_name],mask_bit_position = mask_bit_position,q=q,epochs = 100)
            list_acc += [acc]
        print(list_acc)
import pandas as pd
